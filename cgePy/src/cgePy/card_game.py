...
def loadCardsFilter(cards,filters):

	cardsList = []
	for filter in filters:
		scopeList = filter.split(";")
		namesControl = scopeList[0].split(',')
		valuemin = int(scopeList[1])
		valuemax = int(scopeList[2])
		
		for card in cards:
			if namesControl == ["all"] or card.suit in namesControl :
				print(card.value	)
				if valuemin > valuemax or (card.value >= valuemin and card.value <= valuemax):
